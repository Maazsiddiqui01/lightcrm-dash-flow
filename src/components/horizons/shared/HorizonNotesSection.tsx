...
import { HorizonNote } from '@/hooks/useHorizonNotes';

interface HorizonNotesSectionProps {
  title: string;
  field: 'notes' | 'next_steps';
  currentValue: string | null;
  currentDueDate?: string | null;
  timeline: HorizonNote[];
...
export function HorizonNotesSection({
  title,
  field,
  currentValue,
  currentDueDate,
  ...
  // Sync draft state with current value - extract content without date prefix
  useEffect(() => {
    setDraft('');
  }, [currentValue]);
  ...
  const handleSave = () => {
    if (draft.trim()) {
      ...
      setDraft(''); // Clear draft after saving
    }
  };
              ...
              {isNextSteps && (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor={`add-in-todo-${field}`} className="text-sm font-medium">
                      Add in To Do
                    </Label>
                  </div>
                </div>
