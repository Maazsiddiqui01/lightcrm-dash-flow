...
interface HorizonGpExportDropdownProps {
  data: any[];
  selectedRows: Set<string>;
  visibleColumns?: string[];
}

export function HorizonGpExportDropdown({ data, selectedRows, visibleColumns }: HorizonGpExportDropdownProps) {
  const [isExporting, setIsExporting] = useState(false);
  const { toast } = useToast();

  const getColumnLabel = (key: string): string => {
    const col = LG_HORIZONS_GPS_COLUMNS.find(c => c.name === key);
    return col?.displayName || key;
  };

  const exportToCsv = (rows: any[], filename: string) => {
    const columns = visibleColumns?.filter(c => c !== 'actions') || 
      LG_HORIZONS_GPS_COLUMNS.map(c => c.name).filter(c => !['id', 'created_at', 'updated_at'].includes(c));
  ...
  const handleExportAll = async () => {
    setIsExporting(true);
    try {
      exportToCsv(data, 'horizon-gps-all');
      toast({ title: "Export Complete", description: `Exported ${data.length} GPs` });
    } catch (error) {
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      exportToCsv(selected, 'horizon-gps-selected');
      toast({ title: "Export Complete", description: `Exported ${selected.length} GPs` });
    } catch (error) {
      toast({ title: "Export Failed", description: "Failed to export data", variant: "destructive" });
    } finally {
      setIsExporting(false);
