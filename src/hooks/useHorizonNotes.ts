...
export function useHorizonNotes(
  ...
  // Fetch current notes from the horizon table
  ...
  // Fetch timeline from horizon_notes_timeline view
  ...
  const saveNotesMutation = useMutation({
    mutationFn: async ({ content }: { content: string }) => {
      ...
      const { error } = await addHorizonNote({
        recordId,
        recordType,
        field: 'notes',
        content,
      ...
      if (error) {
        console.error('[useHorizonNotes] RPC Error:', error);
        throw error;
      }
  ...
  const saveNextStepsMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      toast({
        title: "Error",
        description: error?.message || "Failed to save next steps",
        variant: "destructive",
      });
    },
  });
  ...
  return {
    ...
    saveNextSteps: (content: string, dueDate?: string, addInToDo?: boolean) => {
      if (!recordId) return;
      saveNextStepsMutation.mutate({ content, dueDate, addInToDo });
    },
