-- First check and handle the constraint issue by removing duplicates if they exist
DELETE FROM public.lg_focus_area_master 
WHERE focus_area = 'HC: Services (Non-Clinical)' 
AND ctid NOT IN (
    SELECT min(ctid) 
    FROM public.lg_focus_area_master 
    WHERE focus_area = 'HC: Services (Non-Clinical)'
);
...
-- Update master focus area table only if it doesn't already exist
UPDATE public.lg_focus_area_master 
SET focus_area = 'HC: Services (Non-Clinical)' 
WHERE focus_area = 'HC: Non-Clinical Services'
AND NOT EXISTS (
    SELECT 1 FROM public.lg_focus_area_master m2 
    WHERE m2.focus_area = 'HC: Services (Non-Clinical)'
);
