-- Drop and recreate v_contact_email_composer view with email_cc, meeting_cc, and delta_type columns
DROP VIEW IF EXISTS v_contact_email_composer;

CREATE VIEW v_contact_email_composer AS
WITH fa AS (
  SELECT 
    c.id as contact_id,
    c.full_name, 
...
fa_rollup AS (
  ...
  FROM fa
  GROUP BY 
    contact_id, full_name, first_name, email, organization,
    email_cc, meeting_cc, delta_type,
    most_recent_contact, outreach_date
),
opps_combined AS (
  SELECT 
    full_name,
