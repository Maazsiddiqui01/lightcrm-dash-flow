-- Fix v_contact_email_composer to include articles from articles table
...
articles_combined AS (
  -- Fetch articles matching contact's focus areas
  SELECT 
    r.focus_area,
    json_agg(
      jsonb_build_object(
  ...
  FROM (
    SELECT DISTINCT unnest(focus_areas) as focus_area, contact_id
    FROM fa_rollup
  ) r
  INNER JOIN articles a ON a.focus_area = r.focus_area
  WHERE a.last_date_to_use IS NULL OR a.last_date_to_use >= CURRENT_DATE
    ...
    (
      ...
      FROM (
        SELECT 
          ...
          a.last_date_to_use
        FROM articles a
        WHERE a.focus_area = ANY(r.focus_areas)
